...
<style type="text/css">
    ...
    /*table.xinvoice-payment-details{
        background: #f17f33;
          color: #fff;
    }*/
    td.xinvoice-toatl-due {
        background-color: #000;
    }
    td.xinvoice-toatl-due span.xinvoice-total-label {
        color: #fff;
        float: left;
    }
    td.xinvoice-toatl-due span.xinvoice-total-data {
        color: #fff;
        float: right;
    }
    .xinvoice-items tbody{
        background-color: #fff;
    }
    .xtotal-grandtotal tr.xtotal-heading-label {
        text-align: right;
    }
    .xtotal-grandtotal tr.xtotal-heading-data {
        text-align: right;
    }
    .xinvoice-payment-total{
        margin-top: 50px;
    }
    .xinvoice-payment-total .xtotal.xtotal-tax{
        background-color: #fff;
    }
    .xinvoice-payment-total .xtotal.xtotal-grandtotal{
        background-color: #000;
        color: #fff;
    }
    .xinvoice-payment-total .xtotal.xtotal-subtotal{
        background-color: #fff;
    }
    span.xinvoice-total-data {
        font-size: 24px;
    }
    span.xinvoice-total-label {
        font-size: 12px;
        padding: 0px;
    }
    .xinvoice-payment-details td {
        float: right;
    }
</style>
...
<div class="xinvoice-box">
    ...
    <?php if (!isset($data["invoice"]["total_due"]["display"]) || (isset($data["invoice"]["total_due"]["display"]) && $data["invoice"]["total_due"]["display"])) { ?>
    <table cellpadding="0" cellspacing="0" class="xinvoice-invoice-total">
        <tr>
                    ...
                    <?php if (isset($data["total"])) {
                        foreach ($data["total"] as $key => $val) {
                            ...
                            if (isset($val["style"])) $style = $val["style"];
                                if($key == 'grandtotal'){
                                ?>
                                    <tr class="xtotal xtotal-label-<?php echo $key ?>">
                                     ...
                                     <tr class="xtotal xtotal-data-<?php echo $key ?>">
        ...
        <tr>
            <?php if (isset($data["total"])) {
                foreach ($data["total"] as $key => $val) {
                    ...
                    if (isset($val["style"])) $style = $val["style"];
                    ?>
                    <td class="xtotal xtotal-<?php echo $key ?>">
                        <table>
                            <tr class="xtotal-heading-label">
                                <td><?php if (isset($val["label"])) echo "<span class='xinvoice-total-label'>".$val["label"]. "</span>" ?></td>
                            </tr>
                            <tr class="xtotal-heading-data">
                                <td><?php if (isset($val["data"]))  echo "<span class='xinvoice-total-data' style='". $style ."'>".$val["data"]. "</span><br/>"; ?></td>
                            </tr>
                        </table>
                    </td>
                    <?php
